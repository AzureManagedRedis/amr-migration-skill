...
| Clustering policy mismatch | Clustering policy cannot be changed on an existing AMR cache. If the source uses OSS clustering and the target uses EnterpriseCluster (or vice versa), most clients (e.g., StackExchange.Redis) will work without changes. Test the client application and update it if needed. If a different clustering policy is required, create a new AMR cache with the desired policy. |
